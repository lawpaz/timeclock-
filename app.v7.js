...
(function(){
  ...
  const isMacLikeTouch = ((navigator.platform||'').toLowerCase().indexOf('mac') >= 0) && touch;
  if (mobile || isMacLikeTouch || (touch && small)) {
...
function wireNameTabs(){
  ...
  getDocs(collection(db,'users')).then(function(s){
    const arr=[]; s.forEach(function(d){ const x=d.data(); x.id=d.id; arr.push(x); });
    arr.sort(function(a,b){ const na=(fullNameOf(a)||a.username||'').toLowerCase(); const nb=(fullNameOf(b)||b.username||'').toLowerCase(); return na<nb?-1:na>nb?1:0; });
...
function buildEntriesKeyAttr(kind, uid, dk){
...
async function buildEntries(){
  ...
  us.forEach(function(d){ const v=d.data(); userMap.set(d.id,{username:v.username||'', full: fullNameOf(v)||''}); });
  ...
  const rows = Array.from(byDay.values()).sort(function(a,b){
  ...
  rows.forEach(function(r){
    ...
    if(state.isAdmin){
      tdIn  = '<td'+buildEntriesKeyAttr('in', r.uid, r.dateKey)+'>' + (r.firstIn?hmNoLead(r.firstIn):'') + '</td>';
      tdOut = '<td'+buildEntriesKeyAttr('out', r.uid, r.dateKey)+'>' + (r.lastOut?hmNoLead(r.lastOut):'') + '</td>';
      tdDay = '<td'+buildEntriesKeyAttr('daytype', r.uid, r.dateKey)+'>' + (r.dayType||'Work Day') + '</td>';
    }
  ...
  /* Totals (staff view or admin filtered) */
  ...
  if(singleUser){
    ...
    rows.forEach(function(r){ const m=r.totalMins||0; const d=new Date(r.dateKey+'T00:00:00'); if(d<=w1End) minsWeek1+=m; else minsWeek2+=m; });
  ...
  if(state.isAdmin){
    entryRows.querySelectorAll('[data-k="in"],[data-k="out"]').forEach(function(cell){
      cell.addEventListener('keydown', async function(e){
        ...
        let target=null;
        if(kind==='in'){ let min=1e15; docs.forEach(function(d){ if(d.type==='in' && d.ts<min){ min=d.ts; target=d; } }); }
        else { let max=0; docs.forEach(function(d){ if(d.type==='out' && d.ts>max){ max=d.ts; target=d; } }); }
...
function loadUsersForAddEntry(){
  ...
  getDocs(collection(db,'users')).then(function(s){
    const arr=[]; s.forEach(function(d){ const x=d.data(); x.id=d.id; arr.push(x); });
    arr.sort(function(a,b){ const na=(fullNameOf(a)||a.username||'').toLowerCase(); const nb=(fullNameOf(b)||b.username||'').toLowerCase(); return na<nb?-1:na>nb?1:0; });
    arr.forEach(function(u){ const op=document.createElement('option'); op.value=u.id; op.textContent=fullNameOf(u)||u.username||u.email; aeUser.appendChild(op); });
  });
...
function loadStaff(){
  onSnapshot(collection(db,'users'), function(s){
    ...
    staffRows.querySelectorAll('[data-act]').forEach(function(b){ b.addEventListener('click', async function(){
    ...
    staffRows.querySelectorAll('[data-del]').forEach(function(b){ b.addEventListener('click', async function(){
    ...
    staffRows.querySelectorAll('[data-edit]').forEach(function(b){ b.addEventListener('click', async function(){
...
async function computeDailyRows(start,end){
  ...
  const nameByUid = new Map(); usersSnap.forEach(function(u){ const d=u.data(); nameByUid.set(u.id,{full: fullNameOf(d)||'', username:d.username||''}); });
  ...
  const rows = Array.from(perDay.values()).map(function(v){
  ...
  }).sort(function(a,b){ const c=a.name.localeCompare(b.name); return c || (a.date - b.date); });
  return rows;
}
...
async function loadAllReports(){
  ...
  reportList.querySelectorAll('[data-delreport]').forEach(function(b){ b.addEventListener('click', async function(){
...
btnExportTotals.addEventListener('click', function(){
  ...
  const trs=[].slice.call(reportRows.querySelectorAll('tr'));
  trs.forEach(function(tr){
    const t=[].slice.call(tr.querySelectorAll('td')).map(function(x){return x.textContent.trim();});
...
btnExport.addEventListener('click', function(){
  ...
  entryRows.querySelectorAll('tr').forEach(function(tr){
    const t=[].slice.call(tr.querySelectorAll('td')).slice(0,5).map(function(td){return td.textContent.trim();});
    if(t.length===5) lines.push(t.map(function(v){return '"'+v.replace(/"/g,'""')+'"';}).join(','));
  });
...
console.log('TC build v7 loaded');
